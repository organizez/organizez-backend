...
router.get('/getAllCustomersByCategory/:id_category/:iteration', async function(req, res, next) {
    ...
    let iteration = req.params.iteration;
    await connectiondb.query(`select c.id_customer, c.name_company, CONCAT(first_name_representative, ' ', last_name_representative) AS name_representative, c.email_representative, c.phone_representative, c.subscription_type, cs.id_customer_service, cs.name, cs.location, cs.website, cs.company_email, cs.phone, cs.short_description, cs.long_description, cs.image1, cs.image2, cs.image3, cs.image4, cs.image5, cs.image6, cs.image7, cs.image8, cs.image9, cs.image10, cs.image11, cs.image12, cs.image13, cs.image14, cs.image15, cs.image16, cs.image17, cs.image18, cs.image19, cs.image20, cs.minimum_capacity, cs.maximum_capacity, cs.number_hall, sc.category, ct.county from Customers c, Customers_Services cs, Services_Categories sc, Counties ct where c.id_customer = cs.id_customer and cs.id_category = sc.id_category and cs.id_county = ct.id_county and cs.id_category = ${idCategory} LIMIT 15 OFFSET ${iteration};`, (err, rows, fields) => {
        if (err) throw err
        customers = rows;
        res.send(customers);
    })
...
router.get('/getCustomerById/:idCustomer', async function(req, res, next) {
    ...
    await connectiondb.query(`select c.id_customer, c.name_company, first_name_representative, last_name_representative, c.email_representative, c.phone_representative, c.subscription_type, cs.id_customer_service, cs.name, cs.location, cs.website, cs.company_email, cs.phone, cs.short_description, cs.long_description, cs.image1, cs.image2, cs.image3, cs.image4, cs.image5, cs.image6, cs.image7, cs.image8, cs.image9, cs.image10, cs.image11, cs.image12, cs.image13, cs.image14, cs.image15, cs.image16, cs.image17, cs.image18, cs.image19, cs.image20, cs.minimum_capacity, cs.maximum_capacity, cs.number_hall, sc.id_category, sc.category, ct.id_county, ct.county from Customers c, Customers_Services cs, Services_Categories sc, Counties ct where c.id_customer = cs.id_customer and cs.id_category = sc.id_category and cs.id_county = ct.id_county and cs.id_customer_service = ${idCustomer};`, (err, rows, fields) => {
        if (err) throw err
        customer = rows[0];
        res.send(customer);
    })
...
router.post('/addCustomer', async function(req, res, next) {
    ...
    var idCounty = req.body.idCounty;
    ...
    await connectiondb.query(`INSERT INTO Customers(last_name_representative, first_name_representative, phone_representative, email_representative, name_company, subscription_type) VALUES ('${lastNameRepresentative}', '${firstNameRepresentative}', '${phoneRepresentative}', '${emailRepresentative}', '${company}', '${subscriptionType}')`, async(err, rows, fields) => {
        ...
        await connectiondb.query(`INSERT INTO Customers_Services(name, location, website, phone, company_email, short_description, long_description, number_hall, minimum_capacity, maximum_capacity, image1, image2, image3, image4, image5, image6, image7, image8, image9, image10, image11, image12, image13, image14, image15, image16, image17, image18, image19, image20, id_customer, id_category, id_county) VALUES ('${name}', '${location}', '${website}', '${phone}', '${companyEmail}', '${shortDescription}', '${longDescription}', '${numberHall}', '${minimumCapacity}', '${maximumCapacity}', '${image1}', '${image2}', '${image3}', '${image4}', '${image5}', '${image6}','${image7}', '${image8}', '${image9}', '${image10}',  '${image11}', '${image12}', '${image13}', '${image14}', '${image15}', '${image16}','${image17}', '${image18}', '${image19}', '${image20}','${idCustomer}', '${idCategory}', '${idCounty}')`, async(err, rows, fields) => {
            if (err) throw err;
...
router.put('/updateCustomer', async function(req, res, next) {
    ...
    var company = req.body.company;
    var subscriptionType = req.body.subscriptionType;
    var idCustomerService = req.body.idCustomerService;
    var name = req.body.name;
    var location = req.body.location;
    var website = req.body.website;
    var phone = req.body.phone;
    ...
    await connectiondb.query(`UPDATE Customers SET last_name_representative = '${lastNameRepresentative}', first_name_representative = '${firstNameRepresentative}', phone_representative = '${phoneRepresentative}', email_representative = '${emailRepresentative}', name_company = '${company}', subscription_type = '${subscriptionType}' WHERE id_customer = '${idCustomer}';`, async(err, rows, fields) => {
        if (err) throw err;
        await connectiondb.query(`UPDATE Customers_Services SET name = '${name}', location = '${location}', website = '${website}', phone = '${phone}', company_email = '${companyEmail}', short_description = '${shortDescription}', long_description = '${longDescription}', number_hall = '${numberHall}', minimum_capacity =  '${minimumCapacity}', maximum_capacity = '${maximumCapacity}', image1 = '${image1}', image2 = '${image2}', image3 = '${image3}', image4 = '${image4}', image5 = '${image5}', image6 = '${image6}', image7 = '${image7}', image8 = '${image8}', image9 = '${image9}', image10 = '${image10}', image11 = '${image11}', image12 = '${image12}', image13 = '${image13}', image14 = '${image14}', image15 = '${image15}', image16 = '${image16}', image17 = '${image17}', image18 = '${image18}', image19 = '${image19}', image20 = '${image20}', id_customer = '${idCustomer}', id_category = '${idCategory}', id_county = '${idCounty}' WHERE id_customer_service = '${idCustomerService}'`, async(err, rows, fields) => {
            if (err) throw err;
...
router.delete('/deleteCustomer/:idDeletedCustomer/:idDeletedCustomerService', async function(req, res, next) {
    var idCustomer = req.params.idDeletedCustomer;
    var idCustomerService = req.params.idDeletedCustomerService;
    await connectiondb.query(`DELETE FROM Facilities_Options WHERE id_customer_service = '${idCustomerService}'`, async(err, rows, fields) => {
        if (err) throw err;
        await connectiondb.query(`DELETE FROM Customers_Services WHERE id_customer_service = '${idCustomerService}'`, async(err, rows, fields) => {
            if (err) throw err;
            await connectiondb.query(`DELETE FROM Customers WHERE id_customer = '${idCustomer}'`, async(err, rows, fields) => {
                if (err) throw err;
                res.send("succes");
            })
        })
